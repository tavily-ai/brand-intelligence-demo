"""Event handler for research streams — translates Tavily SSE deltas into frontend events."""
...
def process_stream_event(
    ...
    # --- tool calls (Planning / WebSearch / Reflection / Generating) ---
        ...
        if tool_calls.get("type") == "tool_call":
            for tc in tool_calls.get("tool_call", []):
                ...
                if tool_name == "Planning":
                    yield _sse(category, "progress", phase="planning", message="Planning research strategy...")
                elif tool_name == "WebSearch":
                    n = len(queries) if queries else 0
                    yield _sse(category, "progress", phase="searching", message=f"Searching the web ({n} queries)...", queries=queries)
                elif tool_name == "Reflection":
                    yield _sse(category, "progress", phase="analyzing", message="Analyzing findings...")
                elif tool_name == "Generating":
                    yield _sse(category, "progress", phase="generating", message="Generating report...")
            ...
            for tr in tool_calls.get("tool_response", []):
                ...
                if sources:
                    ...
                    yield f'data: {json.dumps({"type": "sources_found", "category": category, "sources": sources_data, "total_sources": len(all_sources)})}\n\n'
        ...
        if isinstance(content, dict):
            for key, value in content.items():
                ...
                if isinstance(value, str):
                    ...
                    accumulated_content[key] += json.dumps(value, ensure_ascii=False)
...
def _sse(category: str, event_type: str, **kwargs) -> str:
    """Build an SSE data line."""
    payload = {"type": event_type, "category": category, **kwargs}
